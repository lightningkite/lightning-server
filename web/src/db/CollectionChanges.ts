...
export class CollectionChanges<T extends any> {
    public constructor(public readonly changes: Array<EntryChange<T>> = []) {
    }
    public static properties = ["changes"]
    public static propertyTypes(T: ReifiedType) { return {changes: [Array, [EntryChange, T]]} }
    copy: (values: Partial<CollectionChanges<T>>) => this;
    equals: (other: any) => boolean;
    hashCode: () => number;
    
    public static pair<T extends any>(old: (T | null) = null, _new: (T | null) = null) {
        let result = new CollectionChanges<T>(old !== null || _new !== null ? [new EntryChange<T>(old, _new)] : []);
...
export function xCollectionChangesMap<T extends any, B extends any>(this_: CollectionChanges<T>, mapper: ((a: T) => B)): CollectionChanges<B> {
    return new CollectionChanges<B>(this_.changes.map((it: EntryChange<T>): EntryChange<B> => (xEntryChangeMap<T, B>(it, mapper))));
}