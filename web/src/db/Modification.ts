...
export namespace Modification {
    ...
    export class OnField<K extends any, V extends any> extends Modification<K> {
        public constructor(public readonly key: KProperty1<K, V>, public readonly modification: Modification<V>) {
            super();
        }
        public static properties = ["key", "modification"]
        public static propertyTypes(K: ReifiedType, V: ReifiedType) { return {key: [KProperty1, K, V], modification: [Modification, V]} }
        copy: (values: Partial<OnField<K, V>>) => this;
        equals: (other: any) => boolean;
        hashCode: () => number;
        
        public invoke(on: K): K {
            return xKProperty1SetCopy<K, V>(this.key, on, this.modification.invoke(on[this.key]));
        }
