...
class InitiatorTest {
    ...
    fun `every subtype survives a round trip`() {
        ...
        Initiator.Task(executionId = execution, causedBy = root, rootExecutionId = root, location = location)
            .roundTripTest()
        Initiator.Schedule(executionId = execution, location = location).roundTripTest()
    ...
    fun `a subtype survives a round trip through the sealed interface`() {
        val initiator: Initiator = Initiator.Task(executionId = execution, location = location)
