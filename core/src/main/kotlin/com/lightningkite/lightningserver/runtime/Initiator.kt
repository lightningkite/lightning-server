...
public sealed interface Initiator {
    ...
    /**
     * The logical request or connection this execution is part of, or null if it is not part of one.
     *
     * One HTTP request is one execution, so for [Http] this is the execution itself. One socket is
     * *five* executions — the lifecycle phases are separate invocations on a serverless engine — so
     * for [WebSocket] it is the socket, not the phase. A task or a schedule tick is not part of a
     * request at all, and says so with null rather than by naming itself.
     *
     * This is the unit an audit request-log row is keyed by, and the unit an access-log line names.
     * It exists because those consumers were each deriving it for themselves — three sites, three
     * spellings of `(this as? WebSocket)?.socketId ?: executionId` — which is how they came to
     * disagree about what a socket's parent was.
     */
    public val logicalId: Uuid?
    ...
    public data class Http @InternalLightningServerApi constructor(
        ...
        /** An HTTP execution is one logical request, and has a row of its own keyed by that id. */
        override val logicalId: Uuid get() = executionId
    ...
    public data class WebSocket @InternalLightningServerApi constructor(
         ...
         * socket, not the phase, is the logical unit a socket's work belongs to and attributes to.
         */
        override val logicalId: Uuid get() = socketId
    ...
    public data class Task @InternalLightningServerApi constructor(
        ...
        /** Not part of a request: it inherits an anchor rather than naming one. */
        override val logicalId: Uuid? get() = null
    }
    ...
    public data class Startup @InternalLightningServerApi constructor(
        ...
        /** Not part of a request at any point, so there is no logical unit to name. */
        override val logicalId: Uuid? get() = null
