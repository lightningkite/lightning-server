...
public class RawWebsocketPath<PATH: PathSpec>(public val pathSegments: PathSegments): HasContextualPath<PATH> {
    ...
    public val match: PathSpecMap.Match<*> get() {
        if (this.matchIfPresent == null) {
            this.matchIfPresent = server.server.endpoints.match(server.externalSerialization.stringArrayFormat, pathSegments) { it.websocket }
        }
        return this.matchIfPresent ?: throw NullPointerException("No match for path: $pathSegments. Registered paths are ${server.server.endpoints.keys}")
    }
