...
@Serializable
@JvmInline
public value class SecretBasis(public val string: String) {
     ...
     */
    public suspend fun derive(key: String): ByteArray =
        CryptographyProvider.Default.get(HMAC).keyDecoder(SHA512).decodeFromByteArray(HMAC.Key.Format.RAW, bytes)
            .signatureGenerator().generateSignature(key.toByteArray())
...
// */
//public fun SecretBasis.hasher(variant: String): Hasher =
//    CryptographyProvider.Default.get(HMAC).keyDecoder(SHA256).decodeFromByteArray().HS512(this.derive(variant))
//
///**
// * Creates an Encryptor from a SecretBasis and a variant.
// */
//public fun SecretBasis.encryptor(variant: String): Encryptor =
//    Encryptor.AesCbcPkcs5Padding(this.derive(variant).sliceArray(0 until 32))
//
///**
