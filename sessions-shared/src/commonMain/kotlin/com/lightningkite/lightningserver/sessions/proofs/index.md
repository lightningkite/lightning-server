...
This package contains the proof-based authentication system for Lightning Server, supporting multi-factor authentication
with pluggable proof methods.

## Authentication Concept

Authentication in Lightning Server uses **proofs** - verified evidence of identity. Each proof method (password, SMS,
email, TOTP, WebAuthn, etc.) provides a proof with an associated strength value. Users must accumulate sufficient proof
strength to authenticate.
...
Each interface defines operations for both establishing credentials (requires auth) and proving credentials (during
login).
