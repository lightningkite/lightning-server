...
fun SecureHasher.signJwt(claims: JwtClaims, json: Json = Json): String = buildString {
    val withDefaults = Json(json) { encodeDefaults = true; explicitNulls = false }
    append(
...
fun SecureHasher.verifyJwt(token: String, json: Json = Json, requiredAudience: String? = null): JwtClaims? {
    val parts = token.split('.')
    if (parts.size != 3) return null  // It's not a JWT, so we'll ignore it.
    val signature = Base64.getUrlDecoder().decode(parts[2])
    val header: JwtHeader = json.decodeFromString(Base64.getUrlDecoder().decode(parts[0]).toString(Charsets.UTF_8))
    val claims: JwtClaims = json.decodeFromString(Base64.getUrlDecoder().decode(parts[1]).toString(Charsets.UTF_8))
