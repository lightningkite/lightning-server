...
@Deprecated(
    ...
    ReplaceWith("path.path(key) bind startupOnce(database, migrateKey = true, action = action)")
)
...
public fun startupOnce(
    ...
    path.path(key) bind startupOnce(database, migrateKey = true, action = action)
}
...
context(_: ServerRuntime)
public fun <SUBJECT : HasId<*>?, T : HasId<ID>, ID : Comparable<ID>> ModelInfo<SUBJECT, T, ID>.baseCollection(): Table<T> =
    baseTable()

@Deprecated("Renamed to table()", ReplaceWith("table()"))
context(_: ServerRuntime)
public fun <SUBJECT : HasId<*>?, T : HasId<ID>, ID : Comparable<ID>> ModelInfo<SUBJECT, T, ID>.collection(): Table<T> =
    table()

@Deprecated("Renamed to table()", ReplaceWith("table(auth)"))
context(_: ServerRuntime)
public suspend fun <SUBJECT : HasId<*>?, T : HasId<ID>, ID : Comparable<ID>> ModelInfo<SUBJECT, T, ID>.collection(auth: AuthAccess<SUBJECT>): Table<T> =
    table(auth)
...
@Deprecated(
    "Use route.arg1 instead",
    ReplaceWith(
        "route.arg1",
        "com.lightningkite.lightningserver.typed.route",
        "com.lightningkite.lightningserver.pathing.arg1"
    )
)
@get:JvmName("path1_1")
...
@Deprecated(
    "Use route.arg2 instead",
    ReplaceWith(
        "route.arg2",
        "com.lightningkite.lightningserver.typed.route",
        "com.lightningkite.lightningserver.pathing.arg2"
    )
)
@get:JvmName("path2_2")
context(runtime: ServerRuntime)
...
@Deprecated(
    "Use route.arg3 instead",
    ReplaceWith(
        "route.second",
        "com.lightningkite.lightningserver.typed.route",
        "com.lightningkite.lightningserver.pathing.arg3"
    )
)
@get:JvmName("path3_3")
context(runtime: ServerRuntime)
