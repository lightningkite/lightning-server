...
resource "aws_s3_bucket" "lambda_bucket" {
  bucket_prefix = "demo-lambda-bucket"
  force_destroy = true
}
...
resource "aws_iam_policy" "bucketDynamoAndInvoke" {
  name        = "demo-bucketDynamoAndInvoke"
  path = "/demo/bucketDynamoAndInvoke/"
  description = "Access to the demo bucket, dynamo, and invoke"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
...
resource "local_sensitive_file" "settings_raw" {
  content = jsonencode({
    ...
    database = {
        url = "postgresql://master:${random_password.database.result}@${aws_rds_cluster.database.endpoint}/demodatabase"
    }
    cache = {
        url = "dynamodb://${var.deployment_location}/demo"
    }
...
resource "aws_lambda_permission" "scheduled_tasks" {
  ...
  source_arn    = "arn:aws:events:${var.deployment_location}:${data.aws_caller_identity.current.account_id}:rule/demo_*"
  lifecycle {
