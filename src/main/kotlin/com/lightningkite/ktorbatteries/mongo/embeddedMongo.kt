...
fun testMongo(replFile: File = Files.createTempDirectory("embeddedMongo").toFile(), port: Int = Network.freeServerPort(Network.getLocalHost())): CoroutineClient
    = embeddedMongo(true, replFile, port)
fun embeddedMongo(replFile: File = File("./build/embeddedMongo"), port: Int = 54961): CoroutineClient
    = embeddedMongo(false, replFile, port)

private fun embeddedMongo(deleteAfter: Boolean, replFile: File, port: Int): CoroutineClient {
    ...
    val replFileExisted = replFile.exists()
    replFile.mkdirs()
