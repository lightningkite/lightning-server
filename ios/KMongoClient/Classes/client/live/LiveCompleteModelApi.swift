...
public class LiveCompleteModelApiCompanion {
    public init() {
        //Necessary properties should be initialized now
    }
    public static let INSTANCE = LiveCompleteModelApiCompanion()
    
    public func create<Model : HasId>(root: String, multiplexSocketUrl: String, path: String, token: String) -> LiveCompleteModelApi<Model> {
        return LiveCompleteModelApi<Model>(read: LiveReadModelApi(url: "\(String(kotlin: root))\(String(kotlin: path))", token: token, serializer: Model.self), write: LiveWriteModelApi(url: "\(String(kotlin: root))\(String(kotlin: path))", token: token, serializer: Model.self), observe: LiveObserveModelApiCompanion.INSTANCE.create(multiplexUrl: multiplexSocketUrl, token: token, path: path));
    }
}
