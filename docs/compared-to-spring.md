...
| `@Repository`              | `database().table(tTable)`           | Type-safe query DSL                |
    ...
    val getUser = path.path("users").arg<String>("id").get.api(
        ...
        implementation = { _: Unit ->
            ...
            cache().get<User>(cacheKey) ?: run {
                val user = database().table(userTable).get(Uuid.parse(id))
...
object Server : ServerBuilder() {
    ...
    val createOrder = path.path("orders").post.api(
        ...
        implementation = { order: CreateOrderRequest ->
            val saved = database().table(orderTable).insertOne(order.toOrder())
...
class UserEndpointsTest {
    ...
    fun testGetUser() = runBlocking {
        with(testRunner) {
            // Insert test data
            Server.database().table(userTable).insertOne(
