...
Typed endpoints provide type-safe, documented API endpoints with automatic serialization, validation, and SDK
generation. This is the recommended way to build APIs in Lightning Server.
...
Validation happens automatically before your implementation is called. Invalid input returns 400 Bad Request with
...
Add WebSocket support for real-time updates to your REST endpoints. This allows clients to receive real-time
notifications when models are created, updated, or deleted.
...
Both patterns create the same endpoints, including a WebSocket endpoint at `/posts/rest/updates` (or `/posts/updates`
depending on your path structure) that sends real-time notifications.
...
The typed endpoints automatically expose their schema for SDK generation. See [meta endpoints](meta.md) for accessing
the schema and generating clients.
