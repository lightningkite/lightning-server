...
fun generateConditionDsl() {
    for (entry in conditionTypes) {
        if(entry.useGeneric) {
            println(entry.run {
                """
                    infix fun <K, T${constraint}> DataClassPath<K, T>.$name($field: $fieldType) = mapCondition(Condition.$name($field))
                """.trimIndent()
            })
        } else {
            println(entry.run {
                """
                    infix fun <K> DataClassPath<K, $constraint>.$name($field: $fieldType) = mapCondition(Condition.$name($field))
                """.trimIndent()
            })
        }
    }
...
fun generateModificationDsl() {
    for (entry in modificationTypes) {
        if(entry.useGeneric) {
            println(entry.run {
                """
                    infix fun <K, T${constraint}> DataClassPath<K, T>.$name($field: $fieldType) = mapModification(Modification.$name($field))
                """.trimIndent()
            })
        } else {
            println(entry.run {
                """
                    infix fun <K> DataClassPath<K, $constraint>.$name($field: $fieldType) = mapModification(Modification.$name($field))
                """.trimIndent()
            })
        }
    }
