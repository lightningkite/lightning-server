...
class SerializationTest {
    ...
    @Test fun conditions() {
        ...
        (startChain<LargeTestModel>().list.listAll { it eq 2 }).cycle()
        (startChain<LargeTestModel>().list.listAny { it eq 2 }).cycle()
        (startChain<LargeTestModel>().list.listSizesEquals(2)).cycle()
        (startChain<LargeTestModel>().set.setAll { it eq 2 }).cycle()
        (startChain<LargeTestModel>().set.setAny { it eq 2 }).cycle()
        (startChain<LargeTestModel>().set.setSizesEquals(2)).cycle()
    ...
    @Test fun modifications() {
        ...
        (startChain<LargeTestModel>().list.listRemoveAll { it eq 2 }).cycle()
        (startChain<LargeTestModel>().list.removeAll(listOf(1, 2))).cycle()
        (startChain<LargeTestModel>().list.listDropFirst()).cycle()
        (startChain<LargeTestModel>().list.listDropLast()).cycle()
        (startChain<LargeTestModel>().set.setRemoveAll { it eq 2 }).cycle()
        (startChain<LargeTestModel>().set.removeAll(setOf(1, 2))).cycle()
        (startChain<LargeTestModel>().set.setDropFirst()).cycle()
        (startChain<LargeTestModel>().set.setDropLast()).cycle()
