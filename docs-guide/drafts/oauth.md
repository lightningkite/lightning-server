...
object UserAuth : PrincipalType<User, Uuid> {
    ...
    override suspend fun fetchByProperty(property: String, value: String): User? {
        return when (property) {
            "email" -> Server.database().table<User>()
...
override suspend fun fetchByProperty(property: String, value: String): User? {
    return when (property) {
        "email" -> Server.database().table<User>().findOne(condition { it.email eq value })
            ?: Server.database().table<User>().insertOne(User(email = value))
        else -> super.fetchByProperty(property, value)
    }
}
```
