...
val ProtoBuf.schema get() = ProtoBufSchemaGeneratorAlt(this)
...
public class ProtoBufSchemaGeneratorAlt(val protoBuf: ProtoBuf) {

    private val reverser = HashMap<SerialDescriptor, KSerializer<*>>()
    ...
    public fun generateSchemaText(
        rootSerializer: KSerializer<*>,
        packageName: String? = null,
        options: Map<String, String> = emptyMap()
    ): String = generateSchemaText(listOf(rootSerializer), packageName, options)
    ...
    public fun generateSchemaText(
        ...
        fun KSerializer<*>.prepReverses() {
            reverser[descriptor] = this
            subAndChildSerializers().forEach { it.prepReverses() }
        }
        serializers.forEach { it.prepReverses() }
        val packageNameCorrected =
            packageName?.let { p -> p.checkIsValidFullIdentifier { "Incorrect protobuf package name '$it'" } }
        checkDoubles(serializers.map { it.descriptor })
        val builder = StringBuilder()
        builder.generateProto2SchemaText(serializers.map { it.descriptor }, packageNameCorrected, options)
    ...
    private fun StringBuilder.generateProto2SchemaText(
        ...
        for ((optionName, optionValue) in options) {
            ...
            val safeOptionNameCorrected = safeOptionName.checkIsValidFullIdentifier { "Invalid option name '$it'" }
            append("option ").append(safeOptionNameCorrected).append(" = \"").append(safeOptionValue).appendLine("\";")
        }
    ...
    private fun StringBuilder.generateMessage(messageType: TypeDefinition): List<TypeDefinition> {
        ...
        if (messageType.isSynthetic) {
            ...
            messageName = messageDescriptor.messageOrEnumName.checkIsValidIdentifier {
                "Invalid name for the message in protobuf schema '${messageDescriptor.messageOrEnumName}'. " +
    ...
    private fun StringBuilder.generateMessageField(
        ...
        getChildType: (Int) -> TypeDefinition = {
            parentType.descriptor.getElementDescriptor(it).decontextualize.let(::TypeDefinition)
        },
        getChildNumber: (Int) -> Int = {
            parentType.descriptor.getElementAnnotations(it).filterIsInstance<ProtoNumber>().singleOrNull()?.number
                ?: (it + 1)
        },
        ...
        for (index in 0 until counts) {
            var fieldName = getChildName(index)
            fieldName = fieldName.checkIsValidIdentifier {
            ...
            if (messageDescriptor.isChildOneOfMessage(index)) {
                ...
                val subDescriptor = fieldDescriptor.getElementDescriptor(1).decontextualize.elementDescriptors.map { it.decontextualize }
                ...
                subDescriptor.forEach { desc ->
                    ...
                    generateMessageField(
                        ...
                        getChildType = { desc.elementDescriptors.single().decontextualize.let(::TypeDefinition) },
                        getChildNumber = {
                            desc.getElementAnnotations(0).filterIsInstance<ProtoNumber>().singleOrNull()?.number
                                ?: (it + 1)
                        },
                ...
                nestedTypes += when {
                    fieldDescriptor.isProtobufNamedType -> generateNamedType(
                        fieldDescriptor = messageDescriptor.getElementDescriptor(index).decontextualize,
                ...
                when {
                    !isPackRequested ||
                            ...
                            !fieldDescriptor.getElementDescriptor(0).decontextualize.isPackable // Ignore if the type is not allowed to be packed
                        -> appendLine(';')
    ...
    private fun StringBuilder.generateNamedType(
        ...
        while (unwrappedFieldDescriptor.isInline) {
            unwrappedFieldDescriptor = unwrappedFieldDescriptor.getElementDescriptor(0).decontextualize
        }
        ...
        val typeName: String = when {
            isSealedPolymorphic -> {
                ...
                nestedTypes = unwrappedFieldDescriptor.elementDescriptors.map { TypeDefinition(it.decontextualize) }.toList()
                nestedTypes.forEachIndexed { _, childType ->
                    append(" ".repeat(indent * 2)).append("//   message ")
                        .append(childType.descriptor.messageOrEnumName).append(", serial name '")
...
//        }
    ...
    private fun StringBuilder.generateMapType(messageType: TypeDefinition, index: Int): List<TypeDefinition> {
        val messageDescriptor = messageType.descriptor
        val mapDescriptor = messageDescriptor.getElementDescriptor(index).decontextualize
        val originalMapValueDescriptor = mapDescriptor.getElementDescriptor(1).decontextualize
        ...
        val keyTypeName = mapDescriptor.getElementDescriptor(0).decontextualize.scalarTypeName(mapDescriptor.getElementAnnotations(0))
    ...
    private fun StringBuilder.generateListType(messageType: TypeDefinition, index: Int): List<TypeDefinition> {
        val messageDescriptor = messageType.descriptor
        val collectionDescriptor = messageDescriptor.getElementDescriptor(index).decontextualize
        val originalElementDescriptor = collectionDescriptor.getElementDescriptor(0).decontextualize
        ...
        val elementDescriptor = elementType.descriptor.decontextualize
    ...
    private fun StringBuilder.generateEnum(enumType: TypeDefinition) {
        ...
        var enumName = enumDescriptor.messageOrEnumName
        enumName = enumName.checkIsValidIdentifier {
        ...
        enumDescriptor.elementDescriptors.forEachIndexed { index, element ->
            var elementName = element.decontextualize.protobufEnumElementName
            elementName = elementName.checkIsValidIdentifier {
    ...
    private val SerialDescriptor.isProtobufScalar: Boolean
        get() = (kind is PrimitiveKind)
                || (kind is StructureKind.LIST && getElementDescriptor(0).decontextualize.kind === PrimitiveKind.BYTE)
    ...
    private val SerialDescriptor.isProtobufRepeated: Boolean
        get() = (kind == StructureKind.LIST && getElementDescriptor(0).decontextualize.kind != PrimitiveKind.BYTE)
                || (kind == StructureKind.MAP && !getElementDescriptor(0).decontextualize.isValidMapKey)

    private val SerialDescriptor.isProtobufMap: Boolean
        get() = kind == StructureKind.MAP && getElementDescriptor(0).decontextualize.isValidMapKey
    ...
    private val SerialDescriptor.decontextualize: SerialDescriptor get() {
        return if(kind == SerialKind.CONTEXTUAL) {
            protoBuf.serializersModule.getContextualDescriptor(this) ?: throw IllegalStateException("No contextual serializer available for ${serialName}, but it is required.")
        } else this
    }

    private fun SerialDescriptor.isChildOneOfMessage(index: Int): Boolean =
        this.getElementDescriptor(index).decontextualize.isSealedPolymorphic && this.getElementAnnotations(index)
            .any { it is ProtoOneOf }
    ...
    private fun SerialDescriptor.scalarTypeName(annotations: List<Annotation> = emptyList()): String {
        ...
        if (kind is StructureKind.LIST && getElementDescriptor(0).decontextualize.kind == PrimitiveKind.BYTE) {
            return "bytes"
        }
    ...
    private fun createLegacyMapType(
        ...
        val messageDescriptor = messageType.descriptor
        val fieldDescriptor = messageDescriptor.getElementDescriptor(index).decontextualize
        ...
        val wrapperDescriptor = buildClassSerialDescriptor(wrapperName) {
            element("key", fieldDescriptor.getElementDescriptor(0).decontextualize.notNull)
            element("value", fieldDescriptor.getElementDescriptor(1).decontextualize.notNull)
        }
    ...
    private fun createNestedCollectionType(
        ...
        val wrapperName = "${messageName}_${fieldName}"
        val wrapperDescriptor = buildClassSerialDescriptor(wrapperName) {
            element("value", elementDescriptor.decontextualize.notNull.decontextualize)
        }
    ...
    private fun String.checkIsValidFullIdentifier(messageSupplier: (String) -> String): String {
        return split('.').joinToString(".") { it.checkIsValidIdentifier { messageSupplier(it) } }
    }

    private fun String.checkIsValidIdentifier(messageSupplier: () -> String): String {
        val validChars = filter { it.isLetterOrDigit() || it == '_' }
        if (validChars.isEmpty()) throw IllegalArgumentException(messageSupplier.invoke())
        if (validChars[0] == '_') {
