...
fun <USER: HasId<*>?, T : HasId<ID>, ID : Comparable<ID>> ModelInfoWithDefault(
    ...
    override suspend fun permissions(auth: AuthAccessor<USER>): ModelPermissions<T> = ModelPermissions.allowAll()
...
typealias ModelInfoWithDefault<USER, T, ID> = ModelInfo<USER, T, ID>
...
fun <USER : HasId<*>?, T : HasId<ID>, ID : Comparable<ID>> modelInfo(
    ...
    val changeListeners = ArrayList<suspend (CollectionChanges<T>) -> Unit>()
    override fun collection(): FieldCollection<T> =
        getCollection(this.baseCollection().withChangeListeners(changeListeners))
