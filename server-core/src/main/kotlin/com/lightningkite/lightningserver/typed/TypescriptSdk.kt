...
fun Documentable.Companion.typescriptSdk(out: Appendable) = with(out) {
    ...
    for (group in groups) {
        ...
        if (groupEndpoints.hasAutoEndpoints()) {
            ...
            append("    readonly ${group.groupToPartName()}: ApiEndpoint<${autoGeneratedInterface}>")
    ...
    userTypes.forEach { userType ->
        ...
        appendLine(
            """
    private generateAutoEndpoints = <M extends {_id: string}, E extends ApiEndpoint<M> = any>(endpoint: E): SessionRestEndpoint<M> => ({
    ...
    private generateCommonEndpoints = <T extends {_id: string}>(path: string): ApiEndpoint<T>  => ({
    ...
    for (group in groups.sortedBy { it.groupToPartName() }) {
        ...
        if (groupEndpoints.hasAutoEndpoints()) {
            ...
            append("        ")
            appendLine("...this.generateCommonEndpoints<${autoGeneratedInterface}>(\"${
                groupEndpoints.first().path.path.escaped.split("/rest").first()
//                group.groupToPartName()
            }\"),")
        }
