...
resource "aws_cloudwatch_log_group" "ws_api" {
  name = "monitoring-ws-gateway-log"
...
resource "aws_iam_policy" "api_gateway_ws" {
  name        = "monitoring-api_gateway_ws"
  path = "/monitoring/api_gateway_ws/"
  description = "Access to the monitoring_api_gateway_ws management"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
