...
fun <T : HasId<ID>, ID> Observable<ListChange<T>>.toListObservable(ordering: Comparator<T>): Observable<List<T>> {
...
fun <T : HasId<ID>, ID> WebSocketIsh<ListChange<T>, Query<T>>.filter(query: Query<T>): Observable<List<T>> =
    this
